...
<pre># config/initializers/flash_messages_helper_settings.rb
ActionView::Base.flash_message_class_proc = lambda {|key| "#{key} hiddable"}</pre>

A _error_ message will now displayed with the dom element class of _error hiddable_ rather then _error_ throughout the application

<pre><div class="error hiddable" id="flash-error">There was an error!</div></pre>
